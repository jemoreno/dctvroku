function ShowGridScreen(selection, backgroundCanvas = invalid, facade = invalid)
	result = invalid

  '	
  ' the content provider class MUST implment GetGridRows and GetItemsForGridRow
  '
  contentProvider = m.content
	...
	lists = contentProvider.GetGridRows()
	...
	for i = 0 to lists.Count() - 1
		...
		if list.items.Count() > 0
			...
			list.items = contentProvider.GetItemsForGridRow(list)
			screen.SetContentList(i, list.items)
		end if
	next
